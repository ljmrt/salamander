...
FileUtils::readFileLines(const char *filePath, std::vector<std::string>& resultFileLines)
{
    std::ifstream fileStream(filePath);

    if (!fileStream.is_open()) {
        throwDebugException("Failed to open file: \"" << filePath << "\".");
    }
...
FileUtils::readFileChars(const char *filePath, bool binaryFile, std::vector<char>& resultFileChars)
{
